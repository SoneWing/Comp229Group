...
import {read, update} from './api-surveysite.js'
...
export default function EditSurveySite ({match}) {
  ...
  const clickSubmit = () => {
    let surveySiteData = new FormData()
    values.name && surveySiteData.append('name', values.name)
    values.description && surveySiteData.append('description', values.description)
    values.image && surveySiteData.append('image', values.image)
    update({
      surveySiteId: match.params.surveySiteId
    }, {
      t: jwt.token
    }, surveySiteData).then((data) => {
      if (data.error) {
        setValues({...values, error: data.error})
      } else {
